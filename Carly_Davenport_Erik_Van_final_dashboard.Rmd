...
### Plotting Covid Case rate (Running total over the past 28 days)
...
### Plotting Covid Test Rate (Running total over the past 28 days)
```{r}
covid_pal <- colorNumeric(
  palette = "Reds",
  domain =
...
```{r, context = "server"}
 observeEvent({
 ...
 },  { #we check for two cases, if X is CaseRate or TestRate, or if X is race or income bracket
   ...
   alameda_grouping_by_zip <-
     ...
     group_by(
       Zip_Number,
       .data[[input$regression_var1]],
       .data[[input$regression_var2]]) %>%
         summarize(estimate = sum(estimate))  %>%
         filter(!(
           Zip_Number %in% c(94720, 95377, 94621, 94603, 94613)
         ))
       
    covid_data_plot <- ggplot(data = alameda_grouping_by_zip,
                                 aes(x = .data[[input$regression_var1]],
                                     y = .data[[input$regression_var2]])) +
         geom_point() +
     ...
     if(input$regression_var1 %in% c(
      ...
      alameda_grouping_by_zip <-
        alameda_data %>%
        group_by(Zip_Number, income, .data[[input$regression_var2]]) %>%
        summarize(estimate = sum(estimate))  %>%
        filter(income %in% input$regression_var1) %>%
      ...
      covid_data_plot <- ggplot(data = alameda_grouping_by_zip,
       ...
       alameda_grouping_by_zip <-
        alameda_data %>%
        group_by(Zip_Number, race, .data[[input$regression_var2]]) %>%
        summarize(estimate = sum(estimate))  %>%
        filter(race %in% input$regression_var1) %>%
        filter(!(Zip_Number %in% c(94720, 95377, 94621, 94603, 94613)))
