...
```{r}
covid_data_plot <- ggplot(
  data = full_covid_data_al,
  aes(
      x = TestRates,
      y = CaseRates
    )
...
covid_data_plot %>% ggplotly()

```

```{r}

model <- lm(CaseRates ~ TestRates, full_covid_data_al)
...
```{r}
al_outliers_removed <- full_covid_data_al[-c(15,53,28,27), ]
covid_data_plot <- ggplot(
  data = al_outlier_removed,
  aes(
      x = TestRates,
      y = CaseRates
    )
...
model <- lm(CaseRates ~ TestRates, al_outliers_removed)
