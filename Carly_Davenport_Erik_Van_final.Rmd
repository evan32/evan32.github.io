...
## Link to dashboard: https://carly17.shinyapps.io/Carly_Davenport_Erik_Van_final_dashboard/
#### Note: 
...
## Mapping Area of Interest: Oakland City in Alameda County
...
## Reflections on Equity Analysis: 
...
# Pre-Regression Findings from some example plots and findings from our dashboard
...
leaflet() %>%
        ...
        addPolygons(data = covidData_oakland, stroke = TRUE, color = "black", weight = 1, smoothFactor = 0.4,
                    fillOpacity = .8,
                    fillColor = ~covid_pal(covidData_oakland_drop_geo[,3]),
                    highlight = highlightOptions(weight = 5, fillOpacity = .95, bringToFront =TRUE),
                    label = paste("Test Rates", covidData_oakland_drop_geo[,3], sep = ": ")) %>%
        addLegend(data = covidData_oakland_drop_geo, title = "Test Rates over <br> past 28 days", pal = covid_pal, 
...
leaflet() %>%
        ...
        addLegend(data = covidData_oakland_drop_geo, title = "Case Rates over <br> past 28 days", pal = covid_pal, 
                  values = covidData_oakland_drop_geo[,2])
```


# Breakdown of Black people in Oakland
...
```{r}
    ...
    race_pal <- colorNumeric(
      ...
      addLegend(
        ...
        title = "Breakdown of Black or African American population in Oakland"
        )
```

# Breakdown of White people in Oakland
...
# Population making under $25,000 annually

```{r}
target_income = c("Less than $10,000", "$10,000 to $14,999", "$15,000 to $19,999", "$20,000 to $24,999")

 oakland_target_income_by_zip <- oakland_data %>% 
       select(Zip_Number, estimate, income) %>%
       group_by(Zip_Number) %>%
       mutate(Zip_Number = as.numeric(Zip_Number)) %>%
      filter(income %in% target_income)  %>%
...
income_pal <- colorNumeric(
      ...
      addLegend(
        ...
        title = "Oakland Income Breakdown"
        )

```

## General Findings Based on the Maps Above: 
...
# Correlation between TestRates and CaseRates with Outliers Removed
...
# Correlation between Case Rates and Low Income (under $25k annually)
```{r}
target_income = c("Less than $10,000", "$10,000 to $14,999", "$15,000 to $19,999", "$20,000 to $24,999")
...
covid_data_plot2 <- ggplot(
  ...
  geom_smooth(method = "lm") +
  labs(
    x = "People with annual household income under $25000",
    y = "CaseRates",
    title = paste0(" Alameda County Linear Regression of Case Rates and <br> People with annual household income under $25000")
  )
...
# Correlation between Test Rates for Black and White Population
```{r}

alameda_grouping_by_zip3 <-
  alameda_data %>% 
  group_by(Zip_Number, race, TestRates) %>%
  filter(race %in% "Black or African American")  %>%
  summarize(estimate = sum(estimate, na.rm = TRUE)) %>%
  filter(!(Zip_Number %in% c(94720,95377, 94621,94603,94613)))

covid_data_plot3 <- ggplot(
  data = alameda_grouping_by_zip3,
  aes(
      x = estimate,
      y = TestRates
    )
  ...
  geom_smooth(method = "lm") +
  labs(
    x = "Number of Black people",
    y = "TestRates",
    title = paste0(" Alameda County Linear Regression of Number of Black People and <br> Test Rates")
  )

covid_data_plot3 %>% ggplotly()

```

```{r}

model <- lm(estimate ~ TestRates, alameda_grouping_by_zip3)
...
covid_data_plot3 <- ggplot(
  ...
  labs(
    ...
    y = "TestRates",
    title = paste0(" Alameda County Linear Regression of Number of White People and <br> Test Rates")
  )
...
model <- lm(estimate ~ TestRates, alameda_grouping_by_zip4)
...
# Overall Analysis of Chosen Regression graphs
